...
//  TODO
//
//  Created by Muhammad Najwan Latief on 19/02/21.
//
...
class iconNames: ObservableObject{
    ...
    init() {
        getAllAlternateIcons()
        if let currentIcons = UIApplication.shared.alternateIconName{
            self.currentIndex = iconNames.firstIndex(of: currentIcons) ?? 0
        }
    }
    
    func getAllAlternateIcons(){
        if let icons = Bundle.main.object(forInfoDictionaryKey: "CFBundleIcons") as? [String: Any],
           let alternativeIcons = icons["CFBundleAlternateIcons"] as? [String: Any]{
